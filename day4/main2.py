...
class GuardLog:
    ...
    def get_best(self, guard_id):
        c = Counter()
        for start, end in self._state[guard_id]:
            c.update(range(start.minute, end.minute))
        return c.most_common()[0]

    def get_most_asleep(self):
        records = {}
        for guard in self._state:
            minute, times = self.get_best(guard)
            records[times] = (guard, minute)

        return max(records.items())[1]
